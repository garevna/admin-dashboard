import { readLocalFile, uploadFile } from '../AJAX'

import { uploads, invalidFileType, uploadFailed } from '../../configs'

export const uploadSLA = async (title, file) => {
  self.postDebugMessage({ file, uploads })
  ...
  const response = await uploadFile.sla({
    title,
    content: loaded.result
  })
